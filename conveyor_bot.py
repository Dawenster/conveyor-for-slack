...
def pending_request_check():
    ...
    if request_queue.status_code == 200:
        ...
        logger.info(
            f"Queue successfully retrieved. It is length: {len(queue_info)}")
    else:
    ...
    for i in range(0, len(queue_info)):
        if queue_info[i]['id'] in request_history:
            to_pop.append(i)
    logger.info(
        f"{len(to_pop)} out of {len(queue_info)} already in history; skipped in queue")
...
def approve_requests(request_id, email, addtl_perms):
    ...
    payload = {'request_id': request_id,
               ...
               'nda_bypass': False}
    do_approval = requests.post(
        apt_url + 'authorizations', headers=apt_head, json=payload)
...
def reject_requests(request_id, email, note):
    ...
    do_rejection = requests.patch(
        apt_url + 'authorization_requests/' + request_id, headers=apt_head, json=payload)
...
def get_selections(payload, selections):
    ...
    for i in range(0, num_selected):
        access_id = selections[block_id[0]
                               ]['perms']['selected_options'][i]['value']
